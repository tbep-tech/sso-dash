...
column(12, 
  sliderInput('wtrsel', 'Select 2-day max sea level increase (m):', min = min(wtrchc), max = max(wtrchc), value = max(wtrchc), step = wtrstp, width = '600px', animate = T)
)
column(12, 
  sliderInput('prpsel', 'Select 2d-precipitation increase (cm):', min = min(prpchc), max = max(prpchc), value = max(prpchc), step = prpstp, width = '600px', animate = T)
)
column(12,
