...
colfun <- colorNumeric(cols, domain = c(0, 365))
                       
# slider choices and steps
wtrchc <- unique(futrsk$water)
wtrstp <- diff(wtrchc) %>% round(2) %>% unique
prpchc <- unique(futrsk$precp)
prpstp <- diff(prpchc) %>% round(2) %>% unique
prbchc <- unique(futrsk$prb)
...
plos <- reactive({
  ...
  p <- ggplot(toplo, aes(x = water, y = precp)) + 
    ...
    scale_fill_gradientn('Days/yr', limits = c(0, 365), colours = cols) + 
...
tsplo <- reactive({
  ...
  p <- plot_ly(toplo) %>%
    add_trace(x = ~yr, y = ~ndays, type = 'scatter', mode = 'none', fill = 'tozeroy', fillcolor = 'red', alpha = alph) %>% 
    layout(
...
Welcome to the SSO future risk assessment dashboard! This tool provides a simple user interface to assess relative risk of future SSO overflows in Pinellas County, Florida.  The probability of overflow risk is a function of coastal water level measured as the two-day maximum and overall precipitation measured as the two-day total.  A simple logistic regression model of the likelihood of overflow vs water level and precipitation provides an estimate of overflow risk, with a user-defined probability for what defines an SSO event.  The future risk is shown as the average number of days per year (+/- 95% confidence interval) for the selected user inputs based on multiple simulated time series over a 200 year period from the present. The plots show the estimated lower confidence interval, average, and upper confidence interval for the 200 year period in the top three panels.  The bottom plot shows the total simulated number of days in each year with overflow events for a 200 year period.  
...
column(12, 
  sliderInput('wtrsel', 'Select 2-day max water increase (m):', min = min(wtrchc), max = max(wtrchc), value = wtrchc[16], step = wtrstp, width = '600px', animate = T)
)
column(12, 
  sliderInput('prpsel', 'Select 2d-precipitation increase (cm):', min = min(prpchc), max = max(prpchc), value = prpchc[13], step = prpstp, width = '600px', animate = T)
)
column(12,
