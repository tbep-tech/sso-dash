...
Welcome to the SSO future risk assessment dashboard! This tool provides a simple user interface to assess relative risk of future SSO overflows in Pinellas County, Florida.  The probability of overflow risk is a function of coastal water level measured as the two-day maximum and overall precipitation measured as the two-day total.  A simple logistic regression model of the likelihood of overflow vs water level and precipitation provides an estimate of overflow risk, with a user-defined probability for what defines an SSO event.  The future risk is shown as the average number of days per year (+/- 95% confidence interval) for the selected user inputs based on multiple simulated time series over a 200 year period from the present.
...
column(12, 
  sliderInput('wtrsel', 'Select 2-day max water increase (m):', min = min(wtrchc), max = max(wtrchc), value = min(wtrchc), step = wtrstp, width = '600px', animate = T)
)
column(12, 
...
This project was funded by a grant from the Tampa Bay Environmental Restoration Fund to [Dr. Mark Luther](mailto:mluther@usf.edu).  For technical questions, please contact [Dr. Steven Meyers](mailto:smeyers@mail.usf.edu) or [Dr. Marcus Beck](mailto:mbeck@tbep.org).  Source code is avaible [here](https://github.com/tbep-tech/sso-dash).
