...
plos <- reactive({
  ...
  p2 <- plot_ly(toplo, x = ~date, y = ~water, type = 'scatter', mode = 'none', fill = 'tozeroy', fillcolor = 'lightblue', alpha = alph) %>% 
    layout(
      yaxis = list(title = '2-day max water (m)', range = c(0, 2.02))
    
    )
  
  p3 <- plot_ly(toplo, x = ~date, y = ~precp, type = 'scatter', mode = 'none', fill = 'tozeroy', fillcolor = 'grey', alpha = alph) %>% 
    layout(
      yaxis = list(title = '2-day precip (cm)', range = c(0, 10.5))
    )
...
Welcome to the SSO risk assessment dashboard! This tool provides a simple user interface to assess relative risk of SSO overflows in Pinellas County, Florida.  The probability of overflow risk (top plot) is a function of coastal water level measured as the two-day maximum (middle plot) and overall precipitation measured as the two-day total (bottom plot).  A simple logistic regression model of the likelihood of overflow vs water level and precipitation provides the output for the top plot, with a user-defined probability (black dotted line) for what defines an SSO event.  The total number of SSO events, total days with events, and percentage of the selected time series with events are summarized on the top. 
...
output$lensbox <- renderValueBox({
  ...
  valueBox(
    lens, "Overflow total duration in days", color = "red"
  )
